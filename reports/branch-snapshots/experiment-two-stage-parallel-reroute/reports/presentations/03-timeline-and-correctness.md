...
這是 diagnostic portfolio、single global strategy，還是 runtime frontier。正式
score 只能來自 single global strategy。

## 投影片 2 - 實驗時期整理

| 日期 | 主要檔案 | 策略狀態 | 可以安全宣稱的內容 |
| --- | --- | --- | --- |
| 2026-05-27 | `../../docs/10-early-source-strategy-report.md`, `../../docs/11-early-results-summary.md` | 早期 source methods：`OpenMP`, fast layer, `P2/P3`, `CUDA`, edge-count。 | 主要作為 method evidence；best-per-benchmark 數字只作 diagnostic，不作正式 score。 |
| 2026-06-04 | `../../docs/23-wl-guard-speedup.md`, `../../docs/24-wl120-single-core-results.md`, `../../docs/25-overall-adaptive-router-results.md` | Net-guided low-layer 與 adaptive repair 開發期。 | 正式可引用的是 one global strategy legal7 `1.771x`；WL portfolio 只作 diagnostic。 |
...
## 投影片 4 - Diagnostic portfolio vs single global strategy

Diagnostic portfolio examples：
...
> Portfolio 是用來看上限和診斷方向；正式 score 必須使用同一套全域策略，
...
| edge-count frontier | 當正式 score | 很快但大多 illegal。 |
...
| earlier same-config requested12 clean rerun `1.345x` | `../03-final-vm-strategy-results.md` |
| current vs same-CLI main `1.677x` | `../04-main-vs-final-router-comparison.md` |
| score definition and portfolio caveat | `../07-scoring-methodology.md` |
...
> 並把 total overflow 降到 166。更快的結果存在，但那些是 diagnostic portfolio、high-WL
