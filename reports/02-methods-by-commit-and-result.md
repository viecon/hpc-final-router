...
- Final scoring follows `07-scoring-methodology.md`: one routing config over one
  fixed benchmark set gets one score. Portfolio/frontier rows below are
  diagnostic only and must not be used as final answers.
...
| Net-guided / low-layer fast assignment | `7ac91ce`, `7300fb7`, `53b1965`, `../docs/23-wl-guard-speedup.md`, `../docs/24-wl120-single-core-results.md` | Give each net a preferred layer and prefer low legal layers before congested fallback. | Diagnostic portfolios: legal7 WL<=1.5 `2.05x`; WL<=1.2 `1.961x`. | WL<=1.5 avg/worst `1.368/1.425`; WL<=1.2 avg/worst `1.125/1.163`. | Useful WL-control direction, but portfolio numbers are not final scores. The 3.585x WL<=1.2 frontier was illegal. |
| P2/P3 budget tuning | early docs, `../docs/10-early-source-strategy-report.md`, `../docs/12-early-vm-speed-frontier.md` | Reduce routing effort via P2/P3 iteration, threshold, box-size tuning. | Diagnostic only: best legal adaptec1 `2.118x`; fastest guarded portfolio `3.17x`. | Fastest guarded original-legal avg/worst WL `1.767/1.862`. | Strong speed lever, but fixed budget is not stable enough without repair and portfolio selection is not a final score. |
...
| CUDA costed maze / dogleg scoring | CUDA commits, `../docs/22-vm-cuda-utilization-log.md`, `../docs/10-early-source-strategy-report.md` | GPU candidate / bounded maze scoring, single and dual GPU probes. | Standalone scorer `33x-38x`; integrated legal adaptec3 `2.780x`. | Often high in early legal rows, e.g. adaptec3 about `1.706x` vs original WL. | Some individual diagnostic rows were legal, but end-to-end GPU utilization stayed near zero. Dual GPU did not improve latency. |
...
| --- | --- | --- | --- | ---: | --- | --- | --- |
| `../docs/11-early-results-summary.md` and `../docs/10-early-source-strategy-report.md` | early source-code strategies | Best-per-benchmark NTHU legal combination using P2/P3 tuning, fast layer, CUDA scoring. | adaptec1-3 | `2.325883x` diagnostic portfolio. | Computed from listed WL: avg about `1.514x`, worst about `1.758x`. | `overflow=0` on those diagnostic rows. | Historical diagnostic only; not one universal strategy and not a final score. |
| `../docs/12-early-vm-speed-frontier.md` | around `58cc00f` | Fastest guarded portfolio: choose fastest optimized row while preserving legality on original-legal cases. | requested 12 | `3.17x` diagnostic portfolio. | All-case avg/worst WL `1.849/2.274`; original-legal avg/worst `1.767/1.862`. | Original-legal guard `7/7`; original-overflow cases still have residual overflow. | Speed frontier only, too much WL and not a single config. |
| `../docs/23-wl-guard-speedup.md` | `69f3eb7` | Net-guided fast greedy layer assignment. | original-legal 7 | `2.05x` diagnostic portfolio. | Avg/worst WL `1.368/1.425`. | `7/7` zero overflow. | Historical WL-control diagnostic, not a final score. |
| `../docs/24-wl120-single-core-results.md` | `7300fb7`, `53b1965` | Low-layer-first net-guided assignment plus legal repair portfolio. | original-legal 7 | `1.961x` diagnostic portfolio. | Avg/worst WL `1.125/1.163`. | `7/7` zero overflow. | Shows WL control can work, but portfolio selection is not a final score. |
