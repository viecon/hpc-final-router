...
out=results/targeted_repairs/nctu_a3_wl20_m5_greedy_edge_split_fast.out
prefix=results/targeted_repairs/nctu_a3_wl20_m5_greedy_edge_split_fast
summary=results/targeted_repairs/nctu_a3_wl20_m5_greedy_edge_split_fast_summary.csv
...
with open(summary, 'w', encoding='utf-8') as f:
    ...
    f.write(','.join([
        'repair_nctu_a3_wl20_m5_edge_split_fast',
...
echo "$job" > results/job_hooks/submitted_repair_nctu_a3_wl20_m5_edge_split_fast.jobid
nohup env WATCH_INTERVAL_SECONDS=20 WATCH_COMMAND="bash scripts/refresh_indexes.sh" \
  bash scripts/watch_job_then_run.sh "$job" "repair-nctu-a3m5-refresh" >/dev/null 2>&1 &
echo "$job"
