...
RESULT_TAG=${RESULT_TAG:-cuda_final_a3_minbatch}
...
BUILD_DIR=${BUILD_DIR:-"$ROOT/external/nthu-route/build-release-openmp-OFF-cuda-ON-radius"}
...
nthu_a3_final_cuda_trust_min128|adaptec3*.gr|$attempts|$cuda_env NTHU_CUDA_DOGLEG_MIN_BATCH=128
nthu_a3_final_cuda_trust_min256|adaptec3*.gr|$attempts|$cuda_env NTHU_CUDA_DOGLEG_MIN_BATCH=256
nthu_a3_final_cuda_trust_min512|adaptec3*.gr|$attempts|$cuda_env NTHU_CUDA_DOGLEG_MIN_BATCH=512
nthu_a3_final_cuda_trust_min1024|adaptec3*.gr|$attempts|$cuda_env NTHU_CUDA_DOGLEG_MIN_BATCH=1024
EOF
...
job=$(
  sbatch --parsable \
    ...
    --mem=64G \
    --time=0-00:30:00 \
    --job-name=nthu-cuda-a3-minbatch \
