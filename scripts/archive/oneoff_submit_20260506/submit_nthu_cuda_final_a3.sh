...
RESULT_TAG=${RESULT_TAG:-cuda_final_a3}
...
nthu_a3_final_cpu_cuda_build|adaptec3*.gr|$attempts|$base_env
nthu_a3_final_cuda_trust|adaptec3*.gr|$attempts|$cuda_env
nthu_a3_final_cuda_gpuonly_r4|adaptec3*.gr|$attempts|$cuda_env NTHU_CUDA_DOGLEG_SKIP_CPU_FALLBACK=1 NTHU_DOGLEG_RADIUS=4
nthu_a3_final_cuda_extra8_r4|adaptec3*.gr|$attempts|$cuda_env NTHU_CUDA_DOGLEG_SKIP_CPU_FALLBACK=1 NTHU_DOGLEG_RADIUS=4 NTHU_DOGLEG_MAX_EXTRA=8
EOF
...
job=$(
  sbatch --parsable \
    ...
    --mem=64G \
    --time=0-00:30:00 \
    --job-name=nthu-cuda-final-a3 \
    --output="$ROOT/logs/%x_%j.log" \
    --error="$ROOT/logs/%x_%j.err" \
    --export=ALL,PROJECT_ROOT="$ROOT",CONTAINER_RUNTIME=apptainer,IMAGE=router.sif,EVALUATOR=lab2,OMP_NUM_THREADS=1,NTHU_OPENMP=OFF,NTHU_CUDA=ON,SKIP_INITIAL_BUILD=0,BUILD_DIR="$ROOT/external/nthu-route/build-release-openmp-OFF-cuda-ON-radius",RUN_TAG="${RESULT_TAG}_summary",OUT="$OUT",ATTEMPTS_OUT="$ATTEMPTS_OUT",ADAPTIVE_VARIANTS_FILE="$variants_file" \
