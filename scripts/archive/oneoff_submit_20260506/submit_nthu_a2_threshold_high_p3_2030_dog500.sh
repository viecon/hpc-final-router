...
RESULT_TAG=${RESULT_TAG:-a2_th_high_p3_2030_dog500}
OUT=${OUT:-"$ROOT/results/nthu_${RESULT_TAG}_summary.csv"}
VARIANTS_FILE=${VARIANTS_FILE:-"$ROOT/results/job_hooks/${RESULT_TAG}.variants"}

{
  for th in 250 251 252 253 254 255 256 258 260; do
    printf 'nthu_a2_th%s_p3_20_30_dog500|--p2-init-box-size=5 --p2-box-expand-size=5 --overflow-threshold=%s --p3-init-box-size=20 --p3-box-expand-size=30|NTHU_DOGLEG_FASTPATH=1 NTHU_DOGLEG_MAX_EXTRA=0 NTHU_DOGLEG_MIN_SCORE=500\n' "$th" "$th"
  done
} > "$VARIANTS_FILE"
