...
mkdir -p logs results/job_hooks results/targeted_repairs/a5_fast_a1_astar
...
out=results/targeted_repairs/a5_fast_a1_astar/repaired.out
prefix=results/targeted_repairs/a5_fast_a1_astar/repair
metrics=results/targeted_repairs/a5_fast_a1_astar/repaired.metrics.json
summary=results/a5_fast_a1_astar_repair_summary.csv
...
echo "$job" > results/job_hooks/submitted_a5_fast_astar_repair.jobid
nohup env WATCH_INTERVAL_SECONDS=20 WATCH_COMMAND="bash scripts/refresh_indexes.sh" \
  bash scripts/watch_job_then_run.sh "$job" "a5_fast_astar_repair-refresh" >/dev/null 2>&1 &
echo "$job"
