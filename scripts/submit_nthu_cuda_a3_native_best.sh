...
RESULT_TAG=${RESULT_TAG:-cuda_a3_native_best}
OUT=${OUT:-"$ROOT/results/nthu_${RESULT_TAG}_summary.csv"}
VARIANTS_FILE=${VARIANTS_FILE:-"$ROOT/results/job_hooks/${RESULT_TAG}.variants"}
BUILD_DIR=${BUILD_DIR:-"$ROOT/external/nthu-route/build-release-openmp-OFF-cuda-ON-native"}
...
base_args="--p2-init-box-size=5 --p2-box-expand-size=5 --overflow-threshold=300000 --p3-max-iteration=8 --p2-max-iteration=4 --p3-init-box-size=35 --p3-box-expand-size=50"

cat > "$VARIANTS_FILE" <<EOF
nthu_cuda_a3_native_step8_p2max4_box35_50|$base_args|$base_env
EOF
...
job=$(
  sbatch --parsable \
    ...
    --mem=64G \
    --time=0-00:30:00 \
    --job-name=nthu-cuda-a3nat \
    --output="$ROOT/logs/%x_%j.log" \
    --error="$ROOT/logs/%x_%j.err" \
    --export=ALL,PROJECT_ROOT="$ROOT",CONTAINER_RUNTIME=apptainer,IMAGE=router.sif,EVALUATOR=lab2,OMP_NUM_THREADS=1,NTHU_OPENMP=OFF,NTHU_CUDA=ON,JOBS=4,BUILD_DIR="$BUILD_DIR",SKIP_INITIAL_BUILD=0,BENCH_PATTERN=adaptec3*.gr,RESULT_SUFFIX="$RESULT_TAG",OUT="$OUT",VARIANTS_FILE="$VARIANTS_FILE",CFLAGS="-O3 -DNDEBUG -march=native",CXXFLAGS="-O3 -DNDEBUG -march=native" \
