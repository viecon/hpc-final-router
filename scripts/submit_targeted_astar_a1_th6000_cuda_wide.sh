...
out=results/targeted_repairs/a1_cuda_th6000_p3big60_score25_astar_wide.out
prefix=results/targeted_repairs/a1_cuda_th6000_p3big60_score25_astar_wide
...
echo "$job" > results/job_hooks/submitted_targeted_astar_a1_th6000_cuda_wide.jobid
nohup env WATCH_INTERVAL_SECONDS=20 WATCH_COMMAND="bash scripts/refresh_indexes.sh" \
  bash scripts/watch_job_then_run.sh "$job" targeted_astar_a1_th6000_cuda_wide-refresh >/dev/null 2>&1 &
echo "$job"
