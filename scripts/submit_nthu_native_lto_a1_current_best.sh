...
RESULT_TAG=${RESULT_TAG:-native_lto_a1_current_best}
OUT=${OUT:-"$ROOT/results/nthu_${RESULT_TAG}_summary.csv"}
VARIANTS_FILE=${VARIANTS_FILE:-"$ROOT/results/job_hooks/${RESULT_TAG}.variants"}
BUILD_DIR=${BUILD_DIR:-"$ROOT/external/nthu-route/build-release-openmp-OFF-native-lto-current"}

cat > "$VARIANTS_FILE" <<'EOF'
nthu_a1_native_lto_fast_layer_th1800|--p2-init-box-size=5 --p2-box-expand-size=5 --overflow-threshold=1800 --p3-init-box-size=20 --p3-box-expand-size=30|NTHU_FAST_GREEDY_LAYER=1
EOF
