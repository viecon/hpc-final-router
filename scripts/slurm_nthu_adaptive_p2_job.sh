...
elif [[ -n "${SLURM_SUBMIT_DIR:-}" && -f "$SLURM_SUBMIT_DIR/scripts/run_nthu_adaptive_p2.sh" ]]; then
...
if [[ "$status" == 0 ]]; then
  ...
  "$CONTAINER_RUNTIME" "${container_exec[@]}" "$IMAGE" bash scripts/run_nthu_adaptive_p2.sh || status=$?
fi
