...
int Congestion::cal_max_overflow() {
    ...
    auto edges = congestionMap2d.all();
    Edge_2d* edge_data = edges.begin();
    const std::ptrdiff_t edge_count = static_cast<std::ptrdiff_t>(congestionMap2d.num_elements());
...
int Congestion::cal_total_wirelength() const {
    int total_wl = 0;
    auto edges = congestionMap2d.all();
    const Edge_2d* edge_data = edges.begin();
    const std::ptrdiff_t edge_count = static_cast<std::ptrdiff_t>(congestionMap2d.num_elements());

#ifdef NTHU_ROUTE_OPENMP
#pragma omp parallel for schedule(static) reduction(+:total_wl)
#endif
    for (std::ptrdiff_t i = 0; i < edge_count; ++i) {
        const Edge_2d& edge = edge_data[i];
