...
void Layer_assignment::fast_greedy_layer_assignment() {
    struct VertexLayer {
        Coordinate_2d vertex;
        int layer;
    };

    std::vector<std::vector<VertexLayer>> net_vertex_layers(output.get_netNumber());
    std::vector<std::unordered_map<long long, int>> preferred_vertex_layer(output.get_netNumber());
    const bool prefer_continuity = std::getenv("NTHU_FAST_GREEDY_LAYER_CONTINUITY") != nullptr;
    auto vertex_key = [](const Coordinate_2d& c) {
        return (static_cast<long long>(c.x) << 32) ^ static_cast<unsigned int>(c.y);
    };

    auto add_xy_edge = [&](int net_id, const Coordinate_2d& a, const Coordinate_2d& b) {
        int best_layer = 0;
        bool found_legal = false;
        double best_score = std::numeric_limits<double>::infinity();

        auto consider_layer = [&](int z) {
            if (z < 0 || z >= output.cur_map_3d.getZSize()) {
                return;
            }
            Edge_3d& edge = output.cur_map_3d.edge(Coordinate_3d { a, z }, Coordinate_3d { b, z });
            if (edge.max_cap <= 0) {
                return;
            }
            const int projected = static_cast<int>(edge.used_net.size() + 1) * 2;
            const bool legal = projected <= edge.max_cap;
            const double score = static_cast<double>(projected) / static_cast<double>(edge.max_cap);
        ...
        if (prefer_continuity) {
            auto& preferred = preferred_vertex_layer[net_id];
            auto a_it = preferred.find(vertex_key(a));
            auto b_it = preferred.find(vertex_key(b));
            if (a_it != preferred.end()) {
                consider_layer(a_it->second);
            }
            if (b_it != preferred.end() && (a_it == preferred.end() || b_it->second != a_it->second)) {
                consider_layer(b_it->second);
            }
        }

        if (!found_legal) {
            for (int z = 0; z < output.cur_map_3d.getZSize(); ++z) {
                consider_layer(z);
            }
        }

        Edge_3d& edge = output.cur_map_3d.edge(Coordinate_3d { a, best_layer }, Coordinate_3d { b, best_layer });
        ++edge.used_net[net_id];
        edge.cur_cap = static_cast<int>(edge.used_net.size()) * 2;
        if (prefer_continuity) {
            auto& preferred = preferred_vertex_layer[net_id];
            preferred[vertex_key(a)] = best_layer;
            preferred[vertex_key(b)] = best_layer;
        }
        net_vertex_layers[net_id].push_back(VertexLayer { a, best_layer });
        net_vertex_layers[net_id].push_back(VertexLayer { b, best_layer });
    };
    ...
    for (int net_id = 0; net_id < static_cast<int>(output.get_netNumber()); ++net_id) {
