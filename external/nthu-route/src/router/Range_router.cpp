...
bool transactional_reroute_batches_enabled() {
    return std::getenv("NTHU_TRANSACTIONAL_REROUTE_BATCHES") != nullptr;
}

bool transactional_l_shape_enabled() {
    const char* value = std::getenv("NTHU_TRANSACTIONAL_LSHAPE");
    return value == nullptr || *value == '\0' || std::atoi(value) != 0;
}

bool transactional_dogleg_enabled() {
    const char* value = std::getenv("NTHU_TRANSACTIONAL_DOGLEG");
    return value == nullptr || *value == '\0' || std::atoi(value) != 0;
}
...
int transactional_serial_repair_max_candidates() {
    const char* value = std::getenv("NTHU_TRANSACTIONAL_SERIAL_REPAIR_MAX_CANDIDATES");
    if (value == nullptr || *value == '\0') {
        return 5000;
    }
...
int transactional_max_candidates() {
    const char* value = std::getenv("NTHU_TRANSACTIONAL_MAX_CANDIDATES");
    if (value == nullptr || *value == '\0') {
        return std::numeric_limits<int>::max();
    }
    const int parsed = std::atoi(value);
...
void NTHUR::RangeRouter::route_twopin_candidates(std::vector<Two_pin_element_2d*>& twopin_list, int version) {
    if (transactional_reroute_batches_enabled() && twopin_list.size() > 1) {
        ...
        const int max_candidates = transactional_max_candidates();
        const int transaction_count = std::min(static_cast<int>(overflow_twopins.size()), max_candidates);
        const int skipped_by_limit = static_cast<int>(overflow_twopins.size()) - transaction_count;
        ...
        const int serial_repair_max_candidates = transactional_serial_repair_max_candidates();
        ...
        remaining.reserve(transaction_count);
        ...
        const int configured_limit = transactional_batch_limit();
        batch_limit = configured_limit > 0 ? std::min(configured_limit, max_threads) : max_threads;
#endif
        ...
        auto make_transaction = [&](Two_pin_element_2d* source,
            ...
            if (try_strict_maze) {
                Coordinate_2d start;
                Coordinate_2d end;
                start.x = min(source->pin1.x, source->pin2.x);
                start.y = min(source->pin1.y, source->pin2.y);
                end.x = max(source->pin1.x, source->pin2.x);
                end.y = max(source->pin1.y, source->pin2.y);
                const int size = construct_2d_tree.BOXSIZE_INC;
                start.x = max(0, start.x - size);
                start.y = max(0, start.y - size);
                end.x = min(construct_2d_tree.rr_map.get_gridx() - 1, end.x + size);
                end.y = min(construct_2d_tree.rr_map.get_gridy() - 1, end.y + size);
        ...
        struct TransactionBatch {
            std::vector<RerouteCandidateBox> items;
            std::unordered_set<int> net_ids;
            int first_order = 0;
        };

        auto batch_can_accept = [&](const TransactionBatch& candidate_batch,
                const RerouteCandidateBox& candidate) {
            if (candidate_batch.net_ids.find(candidate.two_pin->net_id) !=
                    candidate_batch.net_ids.end()) {
                return false;
            }
            for (const RerouteCandidateBox& selected : candidate_batch.items) {
        ...
        auto add_to_batch = [&](TransactionBatch& candidate_batch,
                const RerouteCandidateBox& candidate) {
            candidate_batch.items.push_back(candidate);
            candidate_batch.net_ids.insert(candidate.two_pin->net_id);
        };
...
#ifdef NTHU_ROUTE_OPENMP
        const int scratch_count = std::max(1, std::min(batch_limit, transaction_count));
        std::vector<std::unique_ptr<MonotonicRouting>> monotonic_scratch;
        monotonic_scratch.reserve(scratch_count);
        for (int i = 0; i < scratch_count; ++i) {
            monotonic_scratch.emplace_back(
                    std::unique_ptr<MonotonicRouting>(
                            new MonotonicRouting(congestion, monotonic_enable_flag)));
        }
#else
        MonotonicRouting monotonic_scratch(congestion, monotonic_enable_flag);
#endif

        int batches = 0;
        int parallel_batches = 0;
        int serialized_batches = 0;
        int proposed = 0;
        int committed = 0;
        int invalid = 0;
...
#pragma omp parallel for num_threads(workers) schedule(dynamic, 1)
        ...
        const bool allow_serial_repair = serial_repair_enabled &&
                (serial_repair_max_candidates <= 0 ||
                        static_cast<int>(overflow_twopins.size()) <= serial_repair_max_candidates);
        if (allow_serial_repair) {
        ...
        } else if (serial_repair_enabled) {
            repair_deferred = static_cast<int>(overflow_twopins.size());
        }
        ...
        if (do_profile || parallel_reroute_log_enabled()) {
            log_sp->info("transactional reroute candidates={} overflow_candidates={} transaction_candidates={} skipped_by_limit={} batches={} parallel_batches={} serialized_batches={} proposal_waves={} max_wave_inputs={} proposed={} committed={} invalid={} commit_rejected={} rollback={} clean_skipped={} serial_repair={} repair_skipped={} repair_deferred={} max_batch={} max_workers={} batch_limit={} scheduler={} l_shape={} dogleg={} strict_maze={} strict_maze_area={} virtual_remove=1 serial_repair_enabled={} serial_repair_max_candidates={} fallback=0 plan_ms={:.3f} propose_ms={:.3f} commit_ms={:.3f} repair_ms={:.3f}",
                    ...
                    commit_rejected, rollback, clean_skipped, serial_repair, repair_skipped, repair_deferred,
                    ...
                    strict_maze_area, serial_repair_enabled ? 1 : 0, serial_repair_max_candidates,
    ...
    if (two_stage_parallel_reroute_enabled() && twopin_list.size() > 1) {
        ...
        const int overflow_count = static_cast<int>(overflow_twopins.size());
        const int chunk_limit = std::max(1, std::min(overflow_count, max_parallel_candidates));
        ...
        for (int chunk_begin = 0; chunk_begin < overflow_count; chunk_begin += chunk_limit) {
            ...
            std::vector<TwoStageProposal> proposals(chunk_count);
            ++chunks;
            parallel_candidates += chunk_count;
            max_chunk_size = std::max(max_chunk_size, chunk_count);

            auto make_proposal = [&](int index, MonotonicRouting& local_monotonic) {
                Two_pin_element_2d* source = overflow_twopins[chunk_begin + index];
...
#pragma omp for schedule(dynamic, 1)
        ...
        if (do_profile) {
            range_profile.reroute_ms += profile_ms(reroute_start, ProfileClock::now());
            range_profile.parallel_inputs += parallel_candidates;
            range_profile.parallel_serialized_inputs += serial_fallback;
