...
void readLUT() {
    ...
    for (d = 4; d <= D; d++) {
        if (fscanf(fpwv, "d=%d\n", &d) != 1) {
            fprintf(stderr, "Invalid FLUTE POWV header\n");
            exit(1);
        }
#if ROUTING==1
        if (fscanf(fprt, "d=%d\n", &d) != 1) {
            fprintf(stderr, "Invalid FLUTE POST header\n");
            exit(1);
        }
#endif
        if (d < 0 || d > D) {
            fprintf(stderr, "Invalid FLUTE LUT degree d=%d\n", d);
            exit(1);
        }
        for (k = 0; k < numgrp[d]; k++) {
            ...
            if (ns == 0) {  // same as some previous group
                if (fscanf(fpwv, "%d", &kk) != 1) {
                    fprintf(stderr, "Invalid FLUTE LUT reference record d=%d k=%d\n", d, k);
                    exit(1);
                }
                while ((ch = fgetc(fpwv)) != '\n' && ch != EOF) {
                    ;
                ...
                if (kk < 0 || kk >= MGROUP) {
                    fprintf(stderr, "Invalid FLUTE LUT reference d=%d k=%d kk=%d MGROUP=%d\n", d, k, kk, MGROUP);
                    exit(1);
                }
                ...
                for (i = 1; i <= ns; i++) {
...
#if ROUTING==1
                    nn = 2 * d - 2;
                    if (fread(line, 1, d - 2, fprt) != static_cast<size_t>(d - 2)) {
                        fprintf(stderr, "Invalid FLUTE POST row/col record d=%d k=%d\n", d, k);
                        exit(1);
                    }
                    linep = line;
                    for (j = d; j < nn; j++) {
                    ...
                    if (fread(line, 1, nn / 2 + 1, fprt) != static_cast<size_t>(nn / 2 + 1)) {
                        fprintf(stderr, "Invalid FLUTE POST neighbor record d=%d k=%d\n", d, k);
                        exit(1);
                    }
                    if (line[nn / 2] != '\n') {
                        while ((ch = fgetc(fprt)) != '\n' && ch != EOF) {
